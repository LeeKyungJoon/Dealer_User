...
export default function NoticMain({ route, navigation }) {
  ...
  const _renderItem = ({ item }) => {
    ...
    return (
      ...
      }} delayPressIn={0} style={{paddingHorizontal: scale(15), paddingVertical: scale(12.5), borderBottomColor: "rgba(0, 0, 0, 0.1)",
      borderBottomWidth: 0.5,}}>
        <View style={{flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between'}}>
          <Text style={{...styles.date}}>{moment(item.reg_dt*1000).format("YYYY-MM-DD")}</Text>
          <Image style={{...styles.detailic}} source={require('../../../images/in_ic_68.png')}/>
        </View>
    <Text style={{...styles.title, width: scale(308)}} numberOfLines={1} ellipsizeMode={'tail'}>{item.notice_title}</Text>
      </TouchableOpacity>
    )
  ...
  const _notiList = async () => {
    try {
      let data = await AppServer.CARDEALER_API_00013({ page: _page + 1, range: 30 });
      if (data.success_yn) {
